...
--
-- Host: localhost:3306
-- Generation Time: Jun 05, 2026 at 02:42 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insert_pelanggan` (IN `p_nama` VARCHAR(100), IN `p_nohp` VARCHAR(20), IN `p_alamat` TEXT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_update_pelanggan` (IN `p_id` INT, IN `p_nama` VARCHAR(100), IN `p_nohp` VARCHAR(20), IN `p_alamat` TEXT)   BEGIN
    UPDATE pelanggan
...
--
-- Stand-in structure for view `transaksi_cash`
...
,`diskon` decimal(10,2)
,`metode_bayar` enum('cash','qris','transfer')
,`tanggal_transaksi` datetime
,`status_pembayaran` enum('lunas','dp','belum_bayar')
...
--
-- Stand-in structure for view `transaksi_qris`
-- (See below for the actual view)
--
...
--
-- Stand-in structure for view `transaksi_transfer`
-- (See below for the actual view)
--
...
--
-- Structure for view `transaksi_cash`
--
DROP TABLE IF EXISTS `transaksi_cash`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `transaksi_cash`  AS SELECT `transaksi`.`id_transaksi` AS `id_transaksi`, `transaksi`.`id_booking` AS `id_booking`, `transaksi`.`id_user` AS `id_user`, `transaksi`.`total_bayar` AS `total_bayar`, `transaksi`.`biaya_tambahan` AS `biaya_tambahan`, `transaksi`.`diskon` AS `diskon`, `transaksi`.`metode_bayar` AS `metode_bayar`, `transaksi`.`tanggal_transaksi` AS `tanggal_transaksi`, `transaksi`.`status_pembayaran` AS `status_pembayaran`, `transaksi`.`jumlah_dibayar` AS `jumlah_dibayar` FROM `transaksi` WHERE (`transaksi`.`metode_bayar` = 'cash')  ;

-- --------------------------------------------------------

--
-- Structure for view `transaksi_qris`
--
DROP TABLE IF EXISTS `transaksi_qris`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `transaksi_qris`  AS SELECT `transaksi`.`id_transaksi` AS `id_transaksi`, `transaksi`.`id_booking` AS `id_booking`, `transaksi`.`id_user` AS `id_user`, `transaksi`.`total_bayar` AS `total_bayar`, `transaksi`.`biaya_tambahan` AS `biaya_tambahan`, `transaksi`.`diskon` AS `diskon`, `transaksi`.`metode_bayar` AS `metode_bayar`, `transaksi`.`tanggal_transaksi` AS `tanggal_transaksi`, `transaksi`.`status_pembayaran` AS `status_pembayaran`, `transaksi`.`jumlah_dibayar` AS `jumlah_dibayar` FROM `transaksi` WHERE (`transaksi`.`metode_bayar` = 'qris')  ;

-- --------------------------------------------------------

--
-- Structure for view `transaksi_transfer`
--
DROP TABLE IF EXISTS `transaksi_transfer`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `transaksi_transfer`  AS SELECT `transaksi`.`id_transaksi` AS `id_transaksi`, `transaksi`.`id_booking` AS `id_booking`, `transaksi`.`id_user` AS `id_user`, `transaksi`.`total_bayar` AS `total_bayar`, `transaksi`.`biaya_tambahan` AS `biaya_tambahan`, `transaksi`.`diskon` AS `diskon`, `transaksi`.`metode_bayar` AS `metode_bayar`, `transaksi`.`tanggal_transaksi` AS `tanggal_transaksi`, `transaksi`.`status_pembayaran` AS `status_pembayaran`, `transaksi`.`jumlah_dibayar` AS `jumlah_dibayar` FROM `transaksi` WHERE (`transaksi`.`metode_bayar` = 'transfer')  ;
